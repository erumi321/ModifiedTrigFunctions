...
These requirements can be met by \(\frac{\arccos(\cos(n\theta))}{n}\) \footcite{J. L. Spence, "Perodic absolute value functions"}, which is periodic over \(\period\), has a domain of \(n\theta\in\mathbb{R},\text{and thus } \theta\in\mathbb{R}\), and a range of \([0,\frac{\pi}{s}]\), thus the above equations can be changed to:
\begin{gather*}
\custd(\theta)=\cos(\hp)\sec(\frac{\pi-\arccos(\cos(s\theta))}{s})\\
\cos_s(\theta)=\custd(\theta)\cos(\theta)\\
\sin_s(\theta)=\custd(\theta)\sin(\theta)
